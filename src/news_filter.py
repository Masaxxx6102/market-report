...
もし件数が不足する場合は、重要度が低いニュースや、少し古いニュース（過去24時間以内であれば可）を含めてでも、必ず目標件数を満たしてください。

【優先順位】
1. **監視銘柄の動向**: 以下の監視対象銘柄に関する決算や重要なニュース。
   監視銘柄: {watchlist_str}
2. **マクロ経済・金融政策**: FRBや日銀の要人発言、金利動向、インフレ指標、雇用統計など。
3. **国際・地政学**: 戦争、紛争、地政学的リスク。
4. **メガキャップ・ムーブメント**: 時価総額の大きな企業の個別ニュース。
5. **決算・ガイダンス速報**: 重要決算およびガイダンス。
...
3. 要約：各ニュース日本語で「400〜500文字程度」でプロフェッショナルな深い解説を行ってください。
   単なる事実の羅列ではなく、以下の要素を必ず含めてください。
   - ニュースの背後にある構造的要因や歴史的背景。
   - 現在の市場心理（センチメント）への影響。
   - マクロ経済、特定セクター、および将来の政策（金利等）への具体的な波及効果の予測。
4. 翻訳：マーケット用語を正しく用いた自然で重厚な日本語へ。**タイトル（Headline）も含め、英語や中国語のまま残さず、必ず日本語に翻訳してください。**
